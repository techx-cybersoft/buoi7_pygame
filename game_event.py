...
while running:
    ...
    #Nhận sự kiện từ người dùng
                                  # 0         1     2......
    events = pygame.event.get() # => [event1, event2, ....]
    for event in events:
        #event thoát game
        if event.type == pygame.QUIT:
            running = False
        #event KEYUP => nhấn phím và sau khi thả phím thì nhận event 
        #event KEYDOWN => nhấn phím mỗi lần sẽ nhận mỗi event
        if event.type == pygame.KEYDOWN:
            ...
            if event.key == pygame.K_RIGHT:
                hero_rect.x += 10
        #event MOUSEUP => nhấn chuột và sau khi thả chuột thì nhận event
        #event MOUSEDOWN => nhấn chuột thì nhận event
        if event.type == pygame.MOUSEBUTTONUP:
            #Lấy ra tọa độ chuột
            x, y = pygame.mouse.get_pos() # Vị trí con trỏ chuột khi click
